# Runner: Discrete Morse on a torus oriented with its axis along the x-axis,
# using the vertical height (z-coordinate) as the scalar field
...
# ==============================================
# Torus generator with axis parallel to x-axis
# ==============================================

"""
Generate a wrapped torus surface whose revolution axis is parallel to the x-axis
(i.e., the torus stands "vertically" so height is the z-coordinate).
...
Parameterization (axis = x):
    x = r * sin(v)
    y = (R + r * cos(v)) * cos(u)
    z = (R + r * cos(v)) * sin(u)
with u in [0, 2π), v in [0, 2π).
"""
function generate_torus_axis_x(nu::Int, nv::Int; major_radius::Float64=3.0, minor_radius::Float64=1.0)
    ...
    # Triangles: wrap indices periodically (two per param quad)
    ntris = 2 * nu * nv
    ...
    @inbounds for j in 1:nv
        ...
        for i in 1:nu
            ...
            # 0-based indices for OBJ storage
...
# ==============================================
# UV heatmap with critical point overlay (2D plot)
# ==============================================
...
function main()
    println("Generating torus (axis parallel to x-axis) and running Discrete Morse with height field…")

    # Torus resolution and radii
    nu, nv = 160, 120
    R, r = 3.0, 1.1

    # Build torus oriented with its axis along x
    verts, tris0, u_vals, v_vals = generate_torus_axis_x(nu, nv; major_radius=R, minor_radius=r)

    # Scalar field = height = z-coordinate (ensure a plain Vector{Float64})
    scalar_field = collect(vec(verts[3, :]))

    # Convert triangles to 1-based for TriangleMesh
    tris1 = tris0 .+ 1
    mesh = TriangleMesh(verts, tris1)

    # Run discrete Morse critical point detection; small persistence to reduce noise only
    ...
    save_mesh_obj(verts, tris0, "vertical_torus_height.obj")

    # Plot UV heatmap with critical points overlay (2D visualization)
    draw_uv_heatmap_with_critical_points(
        mesh, minima, saddles, maxima,
        nu, nv, u_vals, v_vals, scalar_field;
        outpath="vertical_torus_uv_height_crit.png",
        jitter=false,
    )

    println("\nDone. OBJ saved as vertical_torus_height.obj")
end
